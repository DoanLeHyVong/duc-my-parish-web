...
export function GenericManagerPage({ resource }: { resource: string }) {
  ...
  return <><div className="admin-title"><div><span>QUẢN LÝ NỘI DUNG</span><h1>{config.title}</h1><p>Thêm, chỉnh sửa và sắp xếp dữ liệu hiển thị.</p></div>{resource !== 'audit-logs' && <button className="button navy" onClick={() => { setEditing(null); setDraft({}); setOpen(true); }}><Plus /> Thêm mới</button>}</div><HomepagePlacementGuide resource={resource} sample={q.data?.[0] || {}} /><div className="admin-card table-wrap"><table><thead><tr><th>ID</th><th>Nội dung</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{q.data?.map((item: any) => <tr key={item.id}><td>#{item.id}</td><td><b>{item.title || item.name || item.key || item.action}</b><small>{item.slug || item.location || item.group || item.createdAt}</small></td><td><span className="status">{item.isActive === false || item.isVisible === false ? 'Ẩn' : 'Hoạt động'}</span></td><td><button onClick={() => { setEditing(item); setDraft(item); setOpen(true); }}><Edit3 /></button><button className="danger" onClick={() => confirm('Bạn chắc chắn muốn xóa?') && remove.mutate(item.id)}><Trash2 /></button></td></tr>)}</tbody></table>{!q.data?.length && <div className="empty-state">Chưa có dữ liệu.</div>}</div>{open && <div className="modal-backdrop"><form className="admin-modal" onSubmit={submit} onChange={(event) => { const target = event.target as unknown as HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement; if (!target.name) return; setDraft((current) => ({ ...current, [target.name]: target instanceof HTMLInputElement && target.type === 'checkbox' ? target.checked : target.value })); }}><div><h2>{editing ? `Chỉnh sửa ${config.title}` : `Thêm ${config.title}`}</h2><button type="button" onClick={() => setOpen(false)}><X /></button></div><HomepagePlacementGuide resource={resource} sample={draft} compact />{config.fields.map((field) => <label key={field.key} className={field.type === 'checkbox' ? 'check-field' : ''}>{field.type === 'checkbox' ? <><input name={field.key} type="checkbox" defaultChecked={editing?.[field.key] ?? true} /> {field.label}</> : <>{field.label}{field.type === 'textarea' ? <textarea name={field.key} rows={5} required={field.required} defaultValue={typeof editing?.[field.key] === 'object' ? JSON.stringify(editing[field.key], null, 2) : editing?.[field.key]} /> : <input name={field.key} type={field.type || 'text'} required={field.required} defaultValue={field.type === 'datetime-local' && editing?.[field.key] ? new Date(editing[field.key]).toISOString().slice(0, 16) : editing?.[field.key]} />}</>}</label>)}<div className="modal-actions"><button type="button" className="button outline" onClick={() => setOpen(false)}>Hủy</button><button className="button navy" disabled={save.isPending}>{save.isPending ? 'Đang lưu…' : 'Lưu thay đổi'}</button></div></form></div>}</>;
}

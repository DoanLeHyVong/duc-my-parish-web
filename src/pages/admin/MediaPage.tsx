...
export function MediaPage() {
  ...
  return <><div className="admin-title"><div><span>THƯ VIỆN CLOUDINARY</span><h1>Media</h1><p>Quản lý hình ảnh dùng trên toàn website.</p></div><button className="button navy" onClick={() => input.current?.click()}><CloudUpload /> Tải ảnh lên</button><input ref={input} hidden multiple accept="image/jpeg,image/png,image/webp,image/gif" type="file" onChange={(e) => e.target.files && upload.mutate(e.target.files)} /></div>{upload.isPending && <div className="upload-progress">Đang tải ảnh lên Cloudinary…</div>}<div className="media-grid">{q.data?.map((media: any) => <article key={media.id}><div>{media.thumbnailUrl || media.secureUrl ? <img src={media.thumbnailUrl || media.secureUrl} alt={media.altText || media.originalName} /> : <Image />}</div><b>{media.originalName}</b><small>{Math.round(media.bytes / 1024)} KB · {media.format}</small><span><button onClick={() => { navigator.clipboard.writeText(media.secureUrl); toast.success('Đã sao chép URL'); }}><Clipboard /></button><button className="danger" onClick={() => confirm('Xóa ảnh khỏi Cloudinary?') && remove.mutate(media.id)}><Trash2 /></button></span></article>)}</div>{!q.data?.length && <div className="empty-state admin-card">Chưa có ảnh. Cloudinary credentials cần được cấu hình trong backend để upload thật.</div>}</>;
}

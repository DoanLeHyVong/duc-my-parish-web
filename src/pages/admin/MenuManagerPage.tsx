...
export function MenuManagerPage() {
  ...
  return <><div className="admin-title"><div><span>ĐIỀU HƯỚNG WEBSITE</span><h1>Menu</h1><p>Kéo thả để đổi thứ tự; hỗ trợ menu cha/con và liên kết nội bộ/ngoài.</p></div><button className="button navy" onClick={() => { setEditing(null); setOpen(true); }}><Plus /> Thêm mục menu</button></div><div className="admin-card menu-manager">{ordered.map((item: any) => <div key={item.id} draggable onDragStart={() => setDragId(item.id)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => drop(e, item.id)}><GripVertical /><span><b>{item.label}</b><small>{item.url} · {item.linkType}</small></span><i className="status">{item.isVisible ? 'Hiện' : 'Ẩn'}</i><button onClick={() => { setEditing(item); setOpen(true); }}><Edit3 /></button><button className="danger" onClick={() => confirm('Xóa mục menu này?') && remove.mutate(item.id)}><Trash2 /></button></div>)}</div>{open && <div className="modal-backdrop"><form className="admin-modal" onSubmit={submit}><div><h2>{editing ? 'Chỉnh sửa mục menu' : 'Thêm mục menu'}</h2><button type="button" onClick={() => setOpen(false)}><X /></button></div><label>Nhãn menu<input name="label" required defaultValue={editing?.label} /></label><label>URL<input name="url" required defaultValue={editing?.url} /></label><label>Loại liên kết<select name="linkType" defaultValue={editing?.linkType || 'INTERNAL'}><option>INTERNAL</option><option>EXTERNAL</option><option>SECTION</option><option>CATEGORY</option><option>POST</option></select></label><label>Menu cha<select name="parentId" defaultValue={editing?.parentId || ''}><option value="">Không có</option>{ordered.filter((i: any) => i.id !== editing?.id).map((i: any) => <option key={i.id} value={i.id}>{i.label}</option>)}</select></label><label>Mở liên kết<select name="target" defaultValue={editing?.target || '_self'}><option value="_self">Cùng cửa sổ</option><option value="_blank">Cửa sổ mới</option></select></label><label className="check-field"><input type="checkbox" name="isVisible" defaultChecked={editing?.isVisible ?? true} /> Hiển thị</label><div className="modal-actions"><button type="button" className="button outline" onClick={() => setOpen(false)}>Hủy</button><button className="button navy">Lưu menu</button></div></form></div>}</>;
}
